document.addEventListener('DOMContentLoaded', () => {
  ...
  function showPlayer(videoId, startTime) {
    ...
    if (youtubePlayer && typeof youtubePlayer.loadVideoById === 'function') {
      ...
      // プレイヤーが存在しない場合：新しいプレイヤーを作成
      youtubePlayer = new YT.Player('youtube-player-mount', { // マウントポイントのID
        height: '100%',
        width: '100%',
        videoId: videoId,
        playerVars: {
  ...
  function hidePlayer() {
    ...
    setTimeout(() => {
      ...
      if (youtubePlayer && typeof youtubePlayer.destroy === 'function') {
        ...
        newIframeContainer.id = 'youtube-player-mount';
        document.getElementById('player-wrapper').appendChild(newIframeContainer);
      }
